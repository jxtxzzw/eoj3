...
class SearchAPI(APIView):
    def get(self, request):
        ...
        if kw:
            ...
            results['problems'] = query_problem(kw, all=is_admin_or_root(request.user))
            results['tag'] = query_tag(kw)
            results['shares'] = query_blog(kw)
            results['contests'] = query_contest(kw)
...
class SearchUserAPI(APIView):
    def get(self, request):
        ...
        results = list()
        if kw:
            if 'contests' in request.GET and request.GET['contests'].isdigit():
                contest = request.GET['contests']
