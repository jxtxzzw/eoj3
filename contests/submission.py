...
from contests.statistics import invalidate_contest_participant
from problems.models import Problem
from problems.statistics import invalidate_problem
from problems.tasks import create_submission
...
from commons.utils import get_permission_for_submission
from commons.utils import is_contest_manager, is_case_download_available
...
class ContestSubmit(BaseContestMixin, View):
    def post(self, request, cid, pid):
        try:
            if self.participate_contest_status < 0 and not self.privileged:  # pending contests
...
class ContestPenaltyDetail(ContestStatus):
    ...
    def get_context_data(self, **kwargs):
        if 'user' not in self.request.GET or 'problems' not in self.request.GET:
            raise PermissionDenied
