...
from contests.base import BaseContestMixin
from contests.statistics import get_participant_score, calculate_problems
from problems.models import Problem
from problems.statistics import get_accept_problem_list, get_attempted_problem_list
...
class DashboardView(BaseContestMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # make sure problems status is correct (for VP purpose)
        if self.virtual_progress is not None:
            calculate_problems(self.contest, self.contest.contest_problem_list, self.virtual_progress)
        # `contests.contest_problem_list` is data["contest_problem_list"]. Same thing.
...
class ContestProblemDetail(BaseContestMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        data['problems'] = data['contest_problem'].problem

        # submit part
        data['lang_choices'] = list(filter(lambda k: k[0] in self.contest.supported_language_list, LANG_CHOICE))
        # if self.request.user.is_authenticated:
        #     data['attempt_left'] = settings.SUBMISSION_ATTEMPT_LIMIT - self.contests.submission_set.filter(
        #         author=self.request.user,
        #         problem_id=data['contest_problem'].problem_id).count()
        #     if self.contests.status != 0:
