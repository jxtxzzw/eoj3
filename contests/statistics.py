...
from problems.statistics import invalidate_problem
...
def calculate_participants(contest: Contest, participants: list, snapshot: timedelta=None):
        ...
        <user_id>: {
            ...
            detail: {
                <problem_id>: {
                    ...
                    score: int (individual score for each problems),
    ...
    Score methods depend on the contests rule:
    1. ACM rule: individual score is all one, total score is solved problems number.
    2. OI rule: individual score is problems weight times the weight of cases passed (round to integer).
    3. CF rule: individual score is problems weight, but will only be available if the solution is by all means correct.

    How much percent one can get is a bit complicated:
    1) The total percent will decrease from contests beginning to contests end from 100% to 50%.
    ...
    for submission in get_submission_filter(contest, snapshot, author_id__in=user_ids):
        ...
        if not contest_problem:  # This problems has been probably deleted
            continue
...
def participants_with_rank(contest: Contest, snapshot: timedelta=None):
    ...
    :param contest:
    :param snapshot:
    :return: contests participants objects with 2 additional fields:
        - actual_rank
        - rank
...
def get_participant_rank(contest: Contest, user_id):
    ...
    Precondition: the contests should be ended FOR THE PARTICIPANT
    """
...
def invalidate_contest_participant(contest: Contest, users=None):
    ...
    The process will get contests participant object from the user instances and save them to database
    """
