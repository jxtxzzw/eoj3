...
from problems.dispatcher.models import Server, ServerProblemStatus
from polygon.models import CodeforcesPackage
from polygon.package.codeforces import get_working_directory
from problems.models import Problem, SpecialProgram, get_input_path, get_output_path
from problems.tasks import upload_problem_to_judge_server
from commons.utils.hash import sha_hash, case_hash
...
class CodeforcesPackageAdapter:
  ...
  def update_info(self, time_limit=None, memory_limit=None, interactive=None):
    ...
    :param interactive: boolean, if problems is interactive, or None if no update required
  ...
  def save_statement_from_file(self, filepath, encoding, language):
    ...
    self.problem.title = content[len('\\begin{problems}{'):content.find('}', len('\\begin{problems}{'))]
    ...
    if notes_start_pos >= 0:
      ...
      content = content[content.find('\n') + 1:]
      self.problem.hint = content[:content.find('\\end{problems}')]
    return True
  ...
  def update(self, update_statement):
    with transaction.atomic():
      path_to_problemxml = os.path.join(self.directory, 'problems.xml')
      if os.path.isfile(path_to_problemxml):
        problem_node = ElementTree.parse(path_to_problemxml)
      else:
        raise RuntimeError("problems.xml not found or couldn't be opened")
