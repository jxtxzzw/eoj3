...
from contests.models import Contest, ContestInvitation, ContestParticipant, ContestClarification, Activity
from contests.statistics import invalidate_contest
from contests.tasks import add_participant_with_invitation
...
from problems.views import StatusList
from submission.util import SubmissionStatus
from commons.utils import write_csv
from commons.utils.download import respond_generate_file
from commons.utils import Identicon
from commons.utils import is_contest_manager
...
class ContestInvitationList(PolygonContestMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        data['contests'] = Contest.objects.get(pk=self.kwargs.get('pk'))
        return data
    ...
    def post(self, request, pk, invitation_pk):
        ...
        try:
            ...
            messages.success(request,
                             'The user <strong>%s</strong> has been successfully added to the contests.' % username)
...
class RejudgeContestProblemSubmission(PolygonContestMixin, View):
    def post(self, request, pk):
        my_problem = request.POST['problems']
        if my_problem == 'all':
