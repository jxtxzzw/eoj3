...
def send_notification(request, **kwargs):
    ...
    comment = kwargs['comment']
    if comment['content_type'].name in ('shares', 'problems', 'contests'):
        recipient = None
        target = None
        verb = 'replied on'
        if comment['content_type'].name == 'shares':
            target = comment['content_object']
            recipient = target.author
            if comment['parent_id']:
                recipient = get_parent_user(comment)
        elif comment['content_type'].name == 'contests':
        ...
        elif comment['content_type'].name == 'problems':
            if comment['parent_id']:
                target = comment['content_object']
                recipient = get_parent_user(comment)
            else:
        ...
        level = 'info'
        if comment['content_type'].name == 'contests':
            level = 'warning'
