...
from accounts.payment import view_report
from accounts.permissions import is_admin_or_root
from problems.dispatcher.models import Server
from problems.statistics import get_accept_problem_list, get_attempted_problem_list, is_problem_accepted
...
from commons.utils import CommentForm
from commons.utils.download import respond_as_attachment
from commons.utils.language import LANG_CHOICE
from commons.utils import is_problem_manager, get_permission_for_submission, is_case_download_available
from commons.utils.site_settings import open_all_protocols
from commons.utils.tagging import edit_string_for_tags
...
class ProblemList(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            ...
            for s in self.request.user.submission_set.select_related("problems"). \
...
class ProblemView(ProblemDetailMixin, TemplateView):
    def get_template_names(self):
        if self.request.GET.get('onlytag'):
            return ['problems/detail/tag.jinja2']
        else:
            return ['problems/detail/problems.jinja2']
...
class StatusList(ListView):
    ...
    def get_queryset(self):
        try:
            ...
            if self.allow_problem_query and 'problems' in self.request.GET:
                queryset = queryset.filter(problem_id=self.reinterpret_problem_identifier(self.request.GET['problems']))
    ...
    def get_context_data(self, **kwargs):
        ...
        data['param_verdict'], data['param_lang'], data['param_user'], data['param_problem'] = \
            ...
            self.request.GET.get('user', ''), self.request.GET.get('problems', '')
...
class ProblemSubmissionView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        data['problems'] = submission.problem
        return data
    ...
    try:
        ...
        if not is_admin_or_root(request.user):
            ...
            if submission.contest_id and not submission.contest.case_public:
                raise PermissionDenied("Case is not public in this contests.")
